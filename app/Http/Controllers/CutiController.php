...
class CutiController extends Controller
{
    ...
    public function pegawai(Request $request, $pegawaiId)
    {
        ...
        return view('cuti.pegawai', compact('pegawai', 'cutis', 'atasanPegawaiMap'));
    }
...
public function destroy(Request $request, $id)
{
    $request->validate([
        ...
        'alasan_hapus.min' => 'Alasan minimal harus 5 karakter.'
    ]);

    $cuti = \App\ordercuti::findOrFail($id);
    $cuti->alasan_hapus = $request->alasan_hapus;
    $cuti->save(); 
    $cuti->delete();

    $url = $request->input('redirect_to') ?: route('ordercuti.index');
