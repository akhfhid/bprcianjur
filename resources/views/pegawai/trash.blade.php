		...
		<table class="table table-bordered table-stripped">
			...
			<tbody>
				@foreach($pegawai as $pegawais)
					...
					<td>
						<form method="POST" action="{{route('pegawai.restore',[$pegawais['id']])}}" class="d-inline">
							@csrf
							<input type="submit" value="Restore" class="btn btn-success btn-sm">
						</form>
						<form method="POST" class="d-inline" onsubmit="return confirm('Delete Data Pegawai Permanently?')" action="{{route('pegawai.delete-permanent',[$pegawais['id']])}}">
							@csrf
