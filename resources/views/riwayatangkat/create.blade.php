...
<div class="col-md-8">
<form enctype="multipart/form-data" class="bg-white shadow-sm p-3" action="{{route('riwayatangkat.store')}}" method="POST">
@csrf
	...
	<select class="form-control" name="speg">
		@foreach ($spegawai as $status =>$name)
		<option value="{{$status}}">{{$name}}</option>
		@endforeach
	</select><br>
	<label>Terhitung Mulai Tanggal</label>
	<input type="date" name="tglangkat" class="form-control"><br>
	<label>Nomor SK Pengangkatan</label>
	<input type="text" name="nosk" class="form-control"><br>
